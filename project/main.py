...
if __name__ == "__main__":
    ...
    # CAN----------------------------------------------------
        ...
        if mode == "online":
            ...
            recursion_frame_check(data_final_can, inter_frame, if_start, sample_rate, 28)
        else:
            ...
            plt.xlabel("time")
            plt.ylabel("CAN_H")
            plt.plot(time_to_decode, can_h)
            plt.show()
            inter_frame = []
            if_start = []  # interframe start
            sample_rate = 25
            recursion_frame_check(can_h, inter_frame, if_start, sample_rate, 28)
    ...
    # LIN----------------------------------------------------
        ...
        if mode == "online":
            ...
            start_of_lin = 0
            for i in range(len(data_final_lin)):
                if data_final_lin[i] == 0:
                    start_of_lin = i
                    break
            plt.xlabel("time")
            plt.ylabel("LIN")
            plt.plot(time_lin[start_of_lin:], data_final_lin[start_of_lin:])
            plt.show()
            lin_id, lin_parity_bits, lin_data, lin_checksum = lin_decoded(
                data_final_lin[start_of_lin:], 50, lin_ver)
    ...
    # I2C----------------------------------------------------
        ...
        if mode == "online":
            ...
            for i in range(len(scl_i2c)):
                if scl_i2c[i] >= 3.0:
                    scl_i2c[i] = 1
                else:
                    scl_i2c[i] = 0

            for i in range(len(sda_i2c)):
                if sda_i2c[i] >= 3.0:
                    sda_i2c[i] = 1
                else:
                    sda_i2c[i] = 0
            ...
            plt.xlabel("TIME")
            plt.ylabel("SDA")
            plt.plot(time_to_decode, sda_i2c)
            plt.subplot(2, 1, 2)
            plt.xlabel("TIME")
            plt.ylabel("SCL")
            plt.plot(time_to_decode, scl_i2c)
            plt.show()
            i2c_decoded(sda_i2c, scl_i2c, 100)
        else:
